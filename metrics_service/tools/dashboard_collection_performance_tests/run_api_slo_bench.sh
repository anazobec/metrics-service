...
# Usage:
#   AWX_DB_PASSWORD=password ./run_api_slo_bench.sh

CONTAINER="automation-metrics-web"
FILL_SCRIPT="/tmp/fill_data.py"
BENCH_SCRIPT="/tmp/benchmark_dashboard_api_slo.py"
...
# runs (see BENCHMARK_JENKINS_PODMAN_API_SLO.md — Security notes).
...
for SCALE in 1 2 3 4; do
  ...
  podman exec \
    ...
    -e DIRECT_DB=true \
    -e BENCHMARK_USER=admin \
    "$CONTAINER" \
    python3.12 manage.py shell -c "
exec(open('$BENCH_SCRIPT').read())
main()
" | tee /home/ansible/results_scale${SCALE}_api_slo.txt
